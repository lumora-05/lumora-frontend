                    ...
                    {canManage ? (
                      <section className="delivery-detail-section delivery-actions-panel">
                        ...
                        {['CHO_TAI_XE_NHAN', 'CHO_BAN_GIAO'].includes(currentStatus) ? (
                          <div className="delivery-action-block">
                            <div className="delivery-assignment-card"><span><Truck size={21} /></span><div><strong>{selectedDelivery.donViVanChuyen || 'Grab (Demo) đang điều phối'}</strong><small>{selectedDelivery.maVanChuyen ? `Mã vận đơn: ${selectedDelivery.maVanChuyen}` : 'Đang chờ cấp mã vận đơn'}</small><small>{selectedDelivery.tenNguoiGiao ? `Tài xế: ${selectedDelivery.tenNguoiGiao}${selectedDelivery.soDienThoaiNguoiGiao ? ` · ${selectedDelivery.soDienThoaiNguoiGiao}` : ''}` : 'Đang chờ thông tin tài xế'}</small></div></div>
                        ...
                        {currentStatus === 'DANG_GIAO' ? (
                          <div className="delivery-action-block"><strong>Grab (Demo) đang giao</strong><small>Trạng thái thật sẽ đi vào webhook. Hai nút dưới đây chỉ mô phỏng callback của đối tác để trình diễn đồ án.</small><button className="success" type="button" disabled={Boolean(actionLoading)} onClick={() => runAction('provider-success', () => deliveryApi.simulateProviderResult(deliveryOrderId(selected), { trangThai: 'GIAO_THANH_CONG', lyDo: null }), 'Grab (Demo) đã báo giao thành công')}><CheckCircle2 size={17} />{actionLoading === 'provider-success' ? 'Đang mô phỏng...' : 'Demo webhook: giao thành công'}</button><label>Lý do giao thất bại<textarea value={form.reason} onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} maxLength={500} placeholder="Không liên lạc được với người nhận..." /></label><button className="danger" type="button" disabled={Boolean(actionLoading) || !form.reason.trim()} onClick={() => runAction('provider-fail', () => deliveryApi.simulateProviderResult(deliveryOrderId(selected), { trangThai: 'GIAO_THAT_BAI', lyDo: form.reason.trim() }), 'Grab (Demo) đã báo giao thất bại')}><XCircle size={17} />{actionLoading === 'provider-fail' ? 'Đang mô phỏng...' : 'Demo webhook: giao thất bại'}</button></div>
                        ) : null}

                        {currentStatus === 'CHO_DOI_SOAT' && isCod ? (
                          <div className="delivery-action-block"><strong>Chờ đối soát COD</strong><small>Grab (Demo) đã báo giao thành công và đã thu hộ tiền từ khách. Chỉ xác nhận khi nhà hàng đã thực sự nhận được khoản COD từ đơn vị vận chuyển.</small><button className="success" type="button" disabled={Boolean(actionLoading) || paymentStatus === 'CHO_HOAN_TIEN'} onClick={() => runAction('complete', () => deliveryApi.complete(deliveryOrderId(selected)), 'Đã xác nhận đối soát COD')}><CheckCircle2 size={17} />{actionLoading === 'complete' ? 'Đang xử lý...' : 'Xác nhận đã đối soát COD'}</button></div>
                        ) : null}

                        {currentStatus === 'CHO_DOI_SOAT' && isVietQr ? (
                          <div className="delivery-action-block"><strong>Đơn VietQR cũ đang chờ ghi nhận nội bộ</strong><small>Trạng thái này chỉ dùng để tương thích dữ liệu đã tạo trước bản cập nhật. Đơn VietQR mới sẽ tự hoàn tất khi Grab (Demo) báo giao thành công.</small><button className="success" type="button" disabled={Boolean(actionLoading) || paymentStatus === 'CHO_HOAN_TIEN'} onClick={() => runAction('complete', () => deliveryApi.complete(deliveryOrderId(selected)), 'Đã ghi nhận hóa đơn')}><CheckCircle2 size={17} />{actionLoading === 'complete' ? 'Đang xử lý...' : 'Ghi nhận hóa đơn'}</button></div>
                        ) : null}

                        {currentStatus === 'GIAO_THAT_BAI' ? (
                          <div className="delivery-action-block"><button type="button" disabled={Boolean(actionLoading)} onClick={() => runAction('retry', () => deliveryApi.retry(deliveryOrderId(selected)), 'Đã yêu cầu Grab (Demo) điều phối lại tài xế')}><RefreshCw size={17} />{actionLoading === 'retry' ? 'Đang xử lý...' : 'Điều phối lại tài xế'}</button></div>
                        ) : null}
