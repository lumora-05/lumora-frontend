...
function DepositActionModal({ item, reason, setReason, busy, onSubmit, onClose }) {
  ...
  return (
    <section className="reservation-manage-modal reservation-deposit-modal" role="dialog" aria-modal="true">
      ...
      <footer><button type="button" onClick={onClose} disabled={busy}>Quay lại</button><button type="button" className="primary" onClick={onSubmit} disabled={busy || !reason.trim()}>{busy ? <LoaderCircle className="spin" size={17} /> : <ReceiptText size={17} />}Xác nhận đã hoàn cọc</button></footer>
    </section>
  );
}
    ...
    <section className={`reservation-manage-page ${useCashierLayout ? 'cashier-reservation-page' : role === 'waiter' ? 'waiter-reservation-page' : ''}`}>
      ...
      {(modal || detail) ? createPortal(
        <div className="reservation-manage-backdrop" onMouseDown={(event) => event.target === event.currentTarget && !busy && (setModal(null), setDetail(null))}>
          ...
          {modal?.action === 'deposit-refund' ? <DepositActionModal action={modal.action} item={modal.item} reason={reason} setReason={setReason} busy={busy} onSubmit={submitAction} onClose={() => setModal(null)} /> : null}
